...
#SBATCH --job-name=mnist_relu6_s42_missing
...
# Run QAT+BR training for MISSING seed 42 runs (4-bit λ=1.0 and λ=10.0)
"$ENV_PY" -u ./A-BR/experiments/mnist_automated_qat_br_sweep.py \
    --baseline-checkpoints-dir ./A-BR/results/mnist_relu6_sweep/checkpoints \
    --seeds 42 \
    --bit-widths 4 \
    --lambdas 1.0 10.0 \
    --qat-epochs 30 \
    ...
    --output-dir ./A-BR/results/mnist_relu6_br_sweep

#!/usr/bin/env python3
"""
CIFAR-10 ResNet18 with QAT + Bin Regularization (AWS/Slurm Optimized)
...
Usage:
    python experiments/cifar10_resnet18_qat_binreg_aws.py \
        ...
        --log-file logs/resnet18_qat_br.log
"""
...
def get_logger(log_file: str, name: str = "resnet18_qat_br") -> logging.Logger:
    """
...
def get_resnet18_cifar10_qat(num_classes=10, pretrained_imagenet=True, clip_value=6.0, num_bits=2, pretrained_baseline=None, logger=None):
    """
...
def main():
    parser = argparse.ArgumentParser(description='CIFAR-10 ResNet18 QAT + Bin Regularization (AWS/Slurm)')
    ...
    parser.add_argument('--log-file', type=str, default='logs/cifar10_resnet18_qat_br.log', help='Log file path')
    ...
    logger.info("="*70)
    logger.info("CIFAR-10 ResNet18 with QAT + Bin Regularization (AWS/Slurm)")
    logger.info("="*70)
    ...
    # Data
    logger.info(f"Loading CIFAR-10 dataset from: {args.data_dir}")
    train_loader, test_loader = get_cifar10_loaders(args.batch_size, data_dir=args.data_dir)
    ...
    logger.info("Model: ResNet18_QAT (adapted for CIFAR-10 + QuantizedClippedReLU)")
    if args.pretrained:
